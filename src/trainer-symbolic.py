...
from utils.dataloader import PatientTCRloader as PatientTCRloader
import argparse
import json
import pandas as pd
...
import os
import sys
from utils.get_representation import quantify_repertoire
from model import aa_encoding_unidirectional as classifier
import time
import datetime
import warnings
...
if __name__ == "__main__":
    try:
        ...
        classifier_model = classifier(indim = 5 if custom_configs["encoding"] == "atchley" else
                                      14 if custom_configs["encoding"] == "aa_prop" else
                                      10 if custom_configs["encoding"] == "kidera" else
                                      5)
        ...
        for e in range(custom_configs["epoch"]):
            ...
            for i in range(len(patient_loader)):
                ...
                all_embeddings = quantify_repertoire(pattcr[1], method = custom_configs["encoding"])
